...
# Remove gevent monkey patching as we are moving away from a gevent-centric model
...
# Remove gevent-specific environment variables
# os.environ['GEVENT_RESOLVER'] = 'thread'
# os.environ['GEVENT_THREADPOOL_SIZE'] = '10'
...
# --- Redis Connection ---
# Standard Redis connection, no longer needs to be gevent-aware
redis_client = redis.Redis(
    host=os.getenv('REDIS_HOST', 'agi-redis-lb'),
    port=6379,
    db=0,
...
class PlannerAgent:
    def __init__(self):
        ...
        self.metrics = get_metrics('agent-planner')
    ...
    def handle_task_created(self, event_type, data, max_retries=3):
        ...
        # 记录EventBus消息指标
        self.metrics.record_eventbus_message(event_type, 'received')
    ...
    def _process_task_with_lock(self, task_id, data, attempt):
        ...
        if not goal:
            ...
            self.metrics.record_task('planning', 'error')
        ...
        try:
            ...
            self.metrics.record_task('planning', status, processing_time)
...
def start_event_listener():
    ...
    global listener_thread, _listener_started
    with _listener_lock:
        if not _listener_started:
            logger.info("🚀 Initializing PlannerAgent for background thread...")
            agent = PlannerAgent()
            listener_thread = threading.Thread(target=agent.start, daemon=True)
            listener_thread.start()
            _listener_started = True
            logger.info("✅ PlannerAgent event listener started in a background thread.")

@app.before_first_request
def initialize_app():
    """Initialize background tasks before the first request."""
    # 设置Prometheus监控
    metrics_port = int(os.getenv('METRICS_PORT', 8080))
    global metrics
    metrics = setup_metrics_for_flask_app(app, 'agent-planner', metrics_port)
...
if __name__ == "__main__":
    ...
    initialize_app()
    # In a direct run, we might want to run the Flask app
    # The daemon thread will exit when the main thread exits
    app.run(host='0.0.0.0', port=int(os.getenv('SERVICE_PORT', 8300)))
